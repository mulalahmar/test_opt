...
def objective_function(x, sign=-1.0):
    return sign * np.dot(data.objective_param, x)

# Define linear constraints
def constraint1(x, sign=-1.0):
    return sign * (np.dot(data.lhs_param[0],x) - data.rhs_bounds[0])

def constraint2(x, sign=-1.0):
    return sign * (np.dot(data.lhs_param[1],x) - data.rhs_bounds[1])
